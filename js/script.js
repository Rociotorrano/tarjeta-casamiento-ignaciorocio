(function () {
  ...
  function confettiBurst(cx, cy, count) {
    ...
    if (!layer) return;
    const colors = ["#b23a4e", "#fcfcfc", "#f2eee4", "#1f3d2b", "#fcfcfc", "#1f3a5f"];
    for (let i = 0; i < count; i++) {
  ...
  function initScratch() {
    ...
    circles.forEach((circle) => {
      ...
      function paint() {
        ...
        ctx.fillStyle = "rgba(252,252,252,0.9)";
  ...
  function initGallery() {
    ...
    const counter = d.getElementById("lb-counter");
    if (!box) return;

    const items = Array.from(d.querySelectorAll("#galeria .g-item img"));
    let idx = 0;
    ...
    d.getElementById("lb-prev").addEventListener("click", (e) => { e.stopPropagation(); show(idx - 1); });
    d.getElementById("lb-next").addEventListener("click", (e) => { e.stopPropagation(); show(idx + 1); });
  ...
  function initPerlas() {
    ...
    const perlaSVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 120" width="120" height="120">
      <path d="M10,110 Q10,50 60,10" fill="none" stroke="#c9a96e" stroke-width="1.2" opacity="0.6"/>
      <circle cx="10" cy="110" r="4" fill="#c9a96e" opacity="0.7"/>
      <circle cx="14" cy="98" r="3.5" fill="#c9a96e" opacity="0.65"/>
      <circle cx="19" cy="86" r="3.2" fill="#c9a96e" opacity="0.6"/>
      <circle cx="25" cy="75" r="3" fill="#c9a96e" opacity="0.55"/>
      <circle cx="32" cy="64" r="2.8" fill="#c9a96e" opacity="0.5"/>
      <circle cx="40" cy="54" r="2.5" fill="#c9a96e" opacity="0.45"/>
      <circle cx="48" cy="44" r="2.2" fill="#c9a96e" opacity="0.4"/>
      <circle cx="56" cy="34" r="2" fill="#c9a96e" opacity="0.35"/>
      <circle cx="60" cy="10" r="1.8" fill="#c9a96e" opacity="0.3"/>
    </svg>`;
    d.querySelectorAll(".section.cream, .section.dark").forEach((sec) => {
      ...
      corners.forEach((cls) => {
        ...
        wrapper.className = "perla-corner " + cls;
        wrapper.setAttribute("aria-hidden", "true");
        wrapper.innerHTML = perlaSVG;
  ...
  /* ---------- Control de acceso (link personal / un solo uso) ---------- */
  function initGate(onPass) {
    ...
    const storageKey = "inv_unico_" + token;
    ...
    // Este dispositivo ya validó el link antes: se permite sin gastarlo de nuevo.
    let yaValidado = false;
    try {
      yaValidado = localStorage.getItem(storageKey) === "1";
    } catch (err) {
      yaValidado = false;
    }
    if (yaValidado) {
    ...
    // Sin backend configurado: modo demo (se permite abrir).
    if (!CONFIG.appsScriptURL) {
    ...
    showGate("¡Hola! Tocá el botón para abrir tu invitación personal.", true);
    openBtn.addEventListener("click", () => {
      openBtn.disabled = true;
      openBtn.innerHTML = '<span class="spinner"></span> Verificando…';
      fetch(CONFIG.appsScriptURL + "?t=" + encodeURIComponent(token))
        ...
        .then((res) => {
          if (res && res.ok) {
            ...
            if (res.nombre && guest) {
              guest.textContent = "Para: " + res.nombre;
              guest.hidden = false;
            }
            pass();
        ...
        .catch(() => {
          ...
          openBtn.textContent = "Reintentar";
          if (msg) {
            msg.textContent =
